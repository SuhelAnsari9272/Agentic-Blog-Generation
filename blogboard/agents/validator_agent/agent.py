...
def validator_node(state: BlogState) -> BlogState:
    ...
    if state.get("dry_run"):
        ...
        return {
            **state, 
            ...
            "md_path": f"{app_settings.storage.TYPE}://dry-run",
    ...
    print(f"  [AGENT] Draft APPROVED! Generating Metadata and Saving to storage ({app_settings.storage.TYPE}).")
    ...
    return {
        **state,
        ...
        "description": description,
        "slug": slug_value,
        "md_path": storage.get_uri(md_relative)
    }
